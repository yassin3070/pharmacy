...
trait SmsTrait {
  ...
  public function sendSms($phone, $msg) {
    ...
    {
      if($this->mora($phone, $msg))
      {
          $count = Setting::where(['key' =>'sms_counter'])->first();
          $count->update(['value' => (int)$count->value + 1]);
      }
  ...
  public function taqnyat($phone, $msg) {
    $data     = SMS::where(['key' => 'taqnyat'])->first();
    // $username = $data->user_name;
    $password = $data->password;
    $sender   = $data->sender_name;
    // 
  ...
  public function mora($phone, $msg) {

    $data     = SMS::where(['key' => 'mora'])->first();
    $username = $data->user_name;
    $password = $data->password;
    $sender   = $data->sender_name;

    // $phone = '966' . ltrim($phone, $phone[0]);

    $url    = 'https://www.mora-sa.com/api/v1/sendsms';
    $fields = array(
        "api_key"  => env('SMS_KEY')??$password,
    ...
    $result = curl_exec($ch);
    $in = json_decode($result, true);
    if($in['status']['code'] == 200)
    {
